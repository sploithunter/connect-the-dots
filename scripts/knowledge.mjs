...
export function validateData(data,profiles,config=readJson('config/network.json')){
 ...
 for(const [id,s] of Object.entries(data.sources)){
  ...
  if(s.archiveUrl!==undefined){try{if(!['https:','http:'].includes(new URL(s.archiveUrl).protocol))throw Error();}catch{fail(`${id}: valid HTTP(S) archive URL required`);}}
 }
