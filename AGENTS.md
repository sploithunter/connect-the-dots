...
Single and split graph modes must share view membership, Network View/search controls, node glyphs and layout behavior. Use the shared modules listed in [wiki/split-graph.md](wiki/split-graph.md). A split pane is another instance of the normal graph, not a separate node-picker product; bridges use the selected graphs without requiring endpoint selection. Connecting routes through seven links must add their intermediate nodes regardless of context depth; depth limits surrounding context. Seven-link routes are tangential. Keep search-budget truncation visible, and preserve this rule when changing split behavior.
