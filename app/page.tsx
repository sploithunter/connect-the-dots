...
const viewNames:Record<string,string>={...Object.fromEntries(Object.entries(network.views).map(([id,v])=>[id,v.label])),path:'Selected path',bridge:'Between two nodes'};
  ...
  <div className="workspace">
   <section className="canvas-section">
    ...
    <div className="explore-controls" aria-label="Graph exploration">
     <div className="explore-actions"><Button variant="outline" disabled={!history.length} onClick={back}>← Back</Button><Button variant="outline" onClick={()=>{if(view!=='bridge')remember();setView('bridge');if(selected)setLeft(selected);setPicking(selected?'right':'left');setRoute(-1);setSelected(null);setActive(null);resetCamera();}}>Compare two nodes</Button>{view!=='overview'&&<Button variant="ghost" onClick={()=>changeView('overview')}>Overview</Button>}</div>
     {selected&&!picking&&<div className="node-actions"><strong>{short(selected)}</strong><Button onClick={()=>neighborhood(selected)}><Focus/>Focus connections</Button><Button variant="outline" onClick={()=>pin('left',selected)}>Use on left</Button><Button variant="outline" onClick={()=>pin('right',selected)}>Use on right</Button></div>}
