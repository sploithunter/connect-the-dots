...
import {expandConnectingGraphs} from '@/lib/connecting-graph.mjs';
...
export default function SplitGraph({initial,focusRef,layout,onNode,onEdge,svgRef}:{initial:Selection&{snapshot:{ids:string[],edgeIds:string[]}},focusRef:React.RefObject<((id:string)=>void)|null>,layout:(ids:string[],edges:Edge[],overview:boolean)=>Point[],onNode:(id:string)=>void,onEdge:(edge:Edge)=>void,svgRef:React.RefObject<SVGSVGElement|null>}){
 ...
 const allowed=useMemo(()=>raw.edges.filter(e=>panes.every(p=>!p.excluded.includes(e.type))),[panes[0].excluded,panes[1].excluded]);
 const expansion=useMemo(()=>expandConnectingGraphs(allowed,[leftGraph,rightGraph],panes.map((p,i)=>p.view==='neighborhood'&&p.anchor?[p.anchor]:[leftGraph,rightGraph][i].ids)),[allowed,leftGraph,rightGraph]);
 const between=useMemo(()=>{
  const visible=new Set(connectionsBetween(allowed,leftGraph.ids,rightGraph.ids).cross.map(c=>c.edge.id));
  for(const edge of expansion.bridge)visible.add(edge.id);
  return connectionsBetween(allowed.filter(e=>visible.has(e.id)),expansion.panes[0].ids,expansion.panes[1].ids);
 },[allowed,expansion,leftGraph,rightGraph]);
 const data={...expansion,...between};
 const leftBase=useMemo(()=>layout(data.panes[0].ids,data.panes[0].edges,panes[0].view==='overview'&&!data.added[0].length),[expansion,layout,panes[0].view]);
 const rightBase=useMemo(()=>layout(data.panes[1].ids,data.panes[1].edges,panes[1].view==='overview'&&!data.added[1].length),[expansion,layout,panes[1].view]);
 ...
 const scene=panes.map((pane,i)=>{
  ...
  const connected=new Set([...(highlight?data.panes[i].edges.filter((e:Edge)=>e.source===highlight||e.target===highlight).flatMap((e:Edge)=>[e.source,e.target]):data.panes[i].ids),...data.bridge.flatMap(e=>[e.source,e.target])]);
 ...
 return <section className="split-explorer" aria-label="Split graph explorer">
  <div className="split-headers">{panes.map((pane,side)=><section className="split-pane-controls" key={side} aria-label={`${sideNames[side]} pane controls`}>
   ...
   <div className="split-pane-actions"><Button variant="outline" disabled={!pane.history.length} onClick={()=>update(side,p=>{const prev=p.history.at(-1)!;return {...fresh(prev.view,prev.anchor,prev.depth,prev.excluded),history:p.history.slice(0,-1)};})}>← Back</Button>{pane.view!=='overview'&&<Button variant="ghost" onClick={()=>go(side,'overview')}>Overview</Button>}{pane.view==='neighborhood'&&<label>Context depth <select aria-label={`${sideNames[side]} depth`} value={pane.depth} onChange={e=>update(side,p=>({...fresh(p.view,p.anchor,Number(e.target.value),p.excluded),history:p.history}))}>{[1,2,3].map(n=><option key={n}>{n}</option>)}</select></label>}</div>
   ...
   <div className="split-pane-actions"><span>{data.panes[side].ids.length} nodes · {data.panes[side].edges.length} connections{data.added[side].length>0&&<small className="connecting-count">Includes {data.added[side].length} added connecting nodes</small>}</span><Button variant="outline" aria-label={`Zoom out ${sideNames[side].toLowerCase()} pane`} onClick={()=>update(side,p=>({...p,zoom:Math.max(.5,p.zoom/1.3)}))}>−</Button><Button variant="outline" aria-label={`Zoom in ${sideNames[side].toLowerCase()} pane`} onClick={()=>update(side,p=>({...p,zoom:Math.min(5,p.zoom*1.3)}))}>+</Button><Button variant="outline" onClick={()=>update(side,p=>({...p,frame:graphBounds(scene[side].points),zoom:1,pan:{x:0,y:0}}))}>Fit graph</Button><Button variant="outline" onClick={()=>reset(side)}>Reset layout</Button></div>
  </section>)}</div>
  <p className="split-help connection-depth-note">Connecting routes are added through 7 links, independent of context depth. Seven-link routes are tangential; depth limits surrounding nodes. Relationship filters still apply.</p>
  <div className="split-summary" aria-live="polite"><strong>{data.cross.length} relationship {data.cross.length===1?'record':'records'} between displayed graphs</strong><span>{data.shared.length} shared entities</span>{data.search.truncated&&<span className="connecting-count" role="status">Search budget reached; some routes within 7 links may be omitted.</span>}{crossings.length<data.cross.length&&<span>{crossings.length} cross-pane records currently on screen; use Fit to see more.</span>}<label><input type="checkbox" checked={showShared} onChange={e=>setShowShared(e.target.checked)}/> Link copies of shared entities</label></div>
  <svg className="split-svg" ref={svgRef} viewBox={`0 0 ${WIDTH*2+GAP} ${HEIGHT}`} role="group" aria-label="Two independently focused graph panes" onPointerMove={move} onPointerUp={()=>{drag.current=null;}} onPointerCancel={()=>{drag.current=null;}} onLostPointerCapture={()=>{drag.current=null;}}>
   ...
   {[0,1].map(side=><g key={side} clipPath={`url(#split-clip-${side})`}>{scene[side].points.map(point=>{const p=scene[side].project(point),scale=scene[side].scale;return <g key={point.id} data-split-node={point.id} data-connection-added={data.added[side].includes(point.id)} data-context-node={([leftGraph,rightGraph][side].ids as string[]).includes(point.id)} data-side={sideNames[side].toLowerCase()} role="button" tabIndex={0} aria-label={`Select ${point.id} in ${sideNames[side].toLowerCase()} pane`} transform={`translate(${p.x+side*(WIDTH+GAP)},${p.y}) scale(${scale})`} onPointerDown={e=>start(e,side,point.id)} opacity={scene[side].highlight&&!scene[side].connected.has(point.id)?.25:1} onMouseEnter={()=>setNodeHover({side,id:point.id})} onMouseLeave={()=>setNodeHover(null)} onClick={()=>{if(!suppress.current)selectNode(side,point.id);}} onDoubleClick={()=>{if(!suppress.current)go(side,'neighborhood',point.id);}} onKeyDown={e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();selectNode(side,point.id);}}} style={{cursor:'grab'}}><GraphNode id={point.id} active={scene[side].highlight===point.id} accent={data.added[side].includes(point.id)}/><title>{data.added[side].includes(point.id)?`${point.id}: included on a connecting route of ${data.search.nodeHops.get(point.id)} links${data.search.nodeHops.get(point.id)===7?' (tangential)':''}`:point.id}</title></g>;})}</g>)}
  </svg>
